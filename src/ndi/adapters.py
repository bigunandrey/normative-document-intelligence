...
def _bbox(value: Any) -> BoundingBox | None:
    if isinstance(value, Mapping):
        ...
        if all(k in value for k in ("x", "y", "width", "height")):
            x, y, w, h = (float(value[k]) for k in ("x", "y", "width", "height")); return BoundingBox(x, y, x+w, y+h)
        for key in ("bbox", "bounding_box", "boundingBox", "prov_bbox"):
            if key in value: return _bbox(value[key])
    if isinstance(value, (list, tuple)) and len(value) == 4: return BoundingBox(*(float(x) for x in value))
    return None
...
def _page(item: Mapping[str, Any]) -> int | None:
    for key in ("page", "page_no", "page_number", "pageNo", "page_num", "page_index"):
        if key in item:
            try:
                value = int(item[key]); return value + 1 if key == "page_index" and value >= 0 else value
...
def _node_type(value: Any) -> NodeType:
    ...
    return {"document":NodeType.DOCUMENT,"page":NodeType.PAGE,"text":NodeType.PARAGRAPH,"paragraph":NodeType.PARAGRAPH,
            "heading":NodeType.SECTION,"title":NodeType.SECTION,"section":NodeType.SECTION,"list":NodeType.LIST,
            "list_item":NodeType.LIST_ITEM,"table":NodeType.TABLE,"row":NodeType.TABLE_ROW,"table_row":NodeType.TABLE_ROW,
            "cell":NodeType.TABLE_CELL,"table_cell":NodeType.TABLE_CELL,"formula":NodeType.FORMULA,"equation":NodeType.FORMULA,
            "note":NodeType.NOTE,"footnote":NodeType.FOOTNOTE,"figure":NodeType.FIGURE,"image":NodeType.FIGURE,
            "header":NodeType.HEADER,"footer":NodeType.FOOTER}.get(raw, NodeType.UNKNOWN)


def new_document(source_name: str, source_sha256: str, page_count: int | None, parser: str, version: str) -> CanonicalDocument:
    return CanonicalDocument(document_id=stable_document_id(source_sha256), source_name=source_name, source_sha256=source_sha256,
                             page_count=page_count, parser_versions={parser: version})
...
def _expand_structural_record(item: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    ...
    result = [item]; typ = _node_type(item.get("type", item.get("element_type", item.get("kind"))))
    if typ == NodeType.TABLE:
        ...
        if isinstance(rows, (list, tuple)):
            for row in rows:
                ...
                row_data.setdefault("type", NodeType.TABLE_ROW.value); row_data.setdefault("page", _page(item))
                row_ordinal = len(result); row_data.setdefault("parent_ordinal", 0); result.append(row_data)
                cells = row_data.get("cells", row_data.get("children", ()))
                if isinstance(cells, (list, tuple)):
                    for cell in cells:
                        cell_data = dict(cell) if isinstance(cell, Mapping) else {"text": _text(cell)}
                        cell_data.setdefault("type", NodeType.TABLE_CELL.value); cell_data.setdefault("page", _page(row_data))
                        cell_data.setdefault("parent_ordinal", row_ordinal); result.append(cell_data)
        ...
        if isinstance(items, (list, tuple)):
            for child in items:
                ...
                child_data.setdefault("type", NodeType.LIST_ITEM.value); child_data.setdefault("page", _page(item))
                child_data.setdefault("parent_ordinal", 0); result.append(child_data)
    return result


def from_records(records: list[Mapping[str, Any]], *, source_name: str, source_sha256: str, page_count: int | None,
                 parser: str, version: str) -> CanonicalDocument:
    document = new_document(source_name, source_sha256, page_count, parser, version); expanded = []
    for item in records:
        start = len(expanded)
        for child in _expand_structural_record(item):
            child_data = dict(child)
            if child_data.get("parent_ordinal") is not None: child_data["parent_ordinal"] = int(child_data["parent_ordinal"]) + start
            expanded.append(child_data)
    node_by_ordinal = {}; pending_parent = {}
    for ordinal, item in enumerate(expanded):
        node = add_observation(document, RawObservation(parser, version, _node_type(item.get("type", item.get("element_type", item.get("kind")))),
                               _text(item.get("text", item.get("content", item.get("value")))), _anchor(item), item.get("confidence"), dict(item)), ordinal=ordinal)
...
def from_markitdown(text: str, *, source_name: str, source_sha256: str, page_count: int | None, version: str = "unknown") -> CanonicalDocument:
    ...
    for line_no, line in enumerate(text.splitlines(), 1):
        value = line.strip()
        if value: records.append({"type": NodeType.SECTION.value if value.startswith("#") else NodeType.TABLE_ROW.value if value.startswith("|") else NodeType.PARAGRAPH.value, "text": value, "source_line": line_no})
